...
function StoryViewer({ trip, onClose }) {
  // Flatten: cover + all highlight photos (supports both old {url} and new {photos:[]} format)
  const photos = [
  ...
  // Deduplicate cover if first highlight is same as cover
  const seen = new Set();
      ...
      >
        ...
        <div style={{ position: 'absolute', top: 62, left: 16, right: 16, zIndex: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,#10B981,#0EA5E9)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>{trip.emoji || '✈️'}</div>
            <div>
...
function HighlightRing({ trip, onOpen }) {
  ...
  // Support both old {url} and new {photos:[]} format
  ...
  return (
    <div className="hl-wrap" onClick={() => onOpen(trip)}>
      <div className={`hl-ring${totalPhotos > 0 ? ' has-photos' : ''}`}>
      ...
      {totalPhotos > 0 && <span style={{ fontSize: 10, color: 'var(--t3)', fontWeight: 600 }}>{totalPhotos} photo{totalPhotos > 1 ? 's' : ''}</span>}
    </div>
  );
}
